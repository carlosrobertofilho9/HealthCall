...
export const CastProvider: React.FC<CastProviderProps> = ({
  applicationId,
  autoJoinPolicy,
  children,
}) => {
  ...
  const initCast = useCallback((isAvailable: boolean) => {
    if (isAvailable && window.cast && window.cast.framework && window.chrome && window.chrome.cast) {
      ...
      const handler = (e: cast.framework.SessionStateChangedEventData) => {
