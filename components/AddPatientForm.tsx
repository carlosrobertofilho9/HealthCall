    ...
    return (
        <div className="lg:col-span-1 bg-[#1a2c22] rounded-2xl p-8 shadow-2xl h-fit">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
                <div>
                    <label className="text-white font-medium mb-2 block" htmlFor="patient-name">Nome do Paciente</label>
                    <div className="relative">
                        <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-[#96c5a9]">person</span>
                        <input className="form-input w-full rounded-full text-white bg-[#264532] border-none h-14 pl-12 pr-4 placeholder:text-[#96c5a9] focus:ring-2 focus:ring-primary transition-all" id="patient-name" placeholder="Digite o nome do paciente" type="text" value={name} onChange={e => setName(e.target.value)} required/>
                    </div>
                </div>
                <div>
                    <label className="text-white font-medium mb-2 block" htmlFor="destination-room">Sala de Destino</label>
                    <div className="relative">
                        <span className="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-[#96c5a9]">meeting_room</span>
                        <select className="form-select appearance-none w-full rounded-full text-white bg-[#264532] border-none h-14 pl-12 pr-10 placeholder:text-[#96c5a9] focus:ring-2 focus:ring-primary transition-all" id="destination-room" value={destination} onChange={e => setDestination(e.target.value)} required>
                            <option value="" disabled>Selecione a sala</option>
                            {DESTINATION_ROOMS.map(room => <option key={room} value={room}>{room}</option>)}
                        </select>
