...
const DisplayPage: React.FC = () => {
  ...
  const [isCalling, setIsCalling] = useState(false); // controla modo "tela cheia de chamada"
  ...
  useEffect(() => {
    ...
    const playBellAndSpeak = async (patient: Patient) => {
      ...
      await new Promise<void>((resolve) => {
        ...
        // fallback se onended não disparar
      ...
      // pequena pausa antes de sair do modo chamada
    ...
    const updateDisplay = () => {
      ...
      if (storedCalledPatient) {
        ...
        if (
          ...
          // toca áudio/voz e alterna layout
    ...
    const initialLoad = () => {
        ...
        if (storedCallHistory) {
          try { setCallHistory(JSON.parse(storedCallHistory)); } catch {}
        }
    };
  ...
  // Layout 1: Tela cheia de chamada (a cada chamada)
  if (isCalling) {
    return (
  ...
  // Layout 2: Visão com "Última chamada" + histórico e próximos abaixo
  return (
    <div className="bg-gray-900 text-white" style={{ fontFamily: '"Spline Sans", "Noto Sans", sans-serif' }}>
      <div className="flex flex-col min-h-screen">
        <header className="px-6 py-4 flex items-center justify-between border-b border-gray-700">
        ...
        <main className="flex-grow p-6 md:p-10 w-full max-w-7xl mx-auto">
          {/* Topo: Última chamada à esquerda, Histórico à direita */}
            ...
            <aside className="bg-gray-800 rounded-2xl p-6">
              ...
              <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-2">
                ...
                {callHistory.map((rec, idx) => (
                  <div key={`${rec.id}-${rec.callCount}-${rec.calledAt}`} className={`flex items-center justify-between rounded-lg px-4 py-3 ${idx === 0 ? 'bg-[#264532]' : 'bg-gray-700/60'}`}>
                    <div>
          ...
          {/* Abaixo: Próximos pacientes em lista */}
